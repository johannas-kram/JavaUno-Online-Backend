...
@SpringBootTest
public class TurnControllerNextTurnTest {
    ...
    @Test
    public void shouldFinalizeTurn() throws Exception {
        String gameUuid = game.getUuid();
        Player player = game.getPlayers().get(0);
        String playerUuid = player.getUuid();
        game.setTurnState(TurnState.FINAL_COUNTDOWN);

        MvcResult mvcResult = this.mockMvc.perform(post("/api/turn/next/{gameUuid}/{playerUuid}", gameUuid, playerUuid))
    ...
    @Test
    public void shouldFailCausedByInvalidLifecycle() throws Exception {
